...
import { collection, query, where, orderBy, onSnapshot, setDoc, doc, updateDoc, deleteDoc } from "firebase/firestore";
import { 
  ResponsiveContainer, 
  AreaChart, 
  Area, 
...
export default function CustomOrderView() {
  ...
  // Load previously submitted orders from Firestore in real-time to satisfy multi-device sync
  useEffect(() => {
    ...
    const loadFromLocal = () => {
      ...
      try {
        ...
        if (isUserAdmin) {
          ...
          setSavedOrders(localOrders.filter(o => o.userId === currentUser.id));
        }
      } catch (e) {
    ...
    let q;
    if (isUserAdmin) {
      q = query(collection(db, "orders"), orderBy("createdAtRaw", "desc"));
    } else {
      q = query(collection(db, "orders"), where("userId", "==", currentUser.id), orderBy("createdAtRaw", "desc"));
    }
    ...
    const unsubscribe = onSnapshot(q, (snapshot) => {
      ...
      try {
        ...
        const newMerged = [...orders];
        cached.forEach(co => {
          if (!newMerged.some(o => o.id === co.id)) {
            newMerged.push(co);
          }
        });
        localStorage.setItem("tuhfa_local_orders", JSON.stringify(newMerged));
      } catch (e) {
        localStorage.setItem("tuhfa_local_orders", JSON.stringify(orders));
      }
    }, (error) => {
      console.warn("Firestore subscription failed, falling back to local orders:", error);
      loadFromLocal();
    });
  ...
  useEffect(() => {
    const isUrlAdmin = window.location.search.toLowerCase().includes("nona") || 
                        window.location.hash.toLowerCase().includes("nona") ||
                        window.location.search.toLowerCase().includes("admin");
    const isStoredAdmin = localStorage.getItem("tuhfa_admin_session") === "true";
    const isUserAdmin = currentUser?.isAdmin === true;

    if (isUrlAdmin || isStoredAdmin || isUserAdmin) {
      setIsAdminUnlocked(true);
      localStorage.setItem("tuhfa_admin_session", "true");
    } else {
  ...
  const handleAdminLogout = () => {
    ...
    localStorage.removeItem("tuhfa_admin_session");
  };
  ...
  const handleDeleteOrder = async (id: string, e: React.MouseEvent) => {
    ...
    if (currentUser && !currentUser.id.startsWith("local_")) {
      try {
        ...
        console.warn("Firestore delete failed, keeping local delete:", err);
      }
    }
  };
    ...
    if (currentUser && !currentUser.id.startsWith("local_")) {
      try {
        ...
        if (order && order.userId) {
          const notifId = "notif_" + Date.now().toString() + "_" + Math.random().toString(36).substring(4);
        ...
        console.warn("Firestore status update failed, keeping local update:", err);
      }
    } else {
      try {
        const order = savedOrders.find(o => o.id === id);
        if (order) {
          ...
          const cachedNotifsStr = localStorage.getItem("tuhfa_local_notifications") || "[]";
          const cachedNotifs = JSON.parse(cachedNotifsStr);
          cachedNotifs.push(newNotif);
          localStorage.setItem("tuhfa_local_notifications", JSON.stringify(cachedNotifs));
        }
      } catch (e) {
  ...
  const handleUpdateOrderPrice = async (id: string, price: number) => {
    ...
    if (currentUser && !currentUser.id.startsWith("local_")) {
      try {
        await updateDoc(doc(db, "orders", id), { priceEGP: price });
      } catch (err) {
        console.warn("Firestore price update failed, keeping local update:", err);
      }
    }
  };
    ...
    try {
      ...
      if (currentUser && !currentUser.id.startsWith("local_")) {
        try {
          ...
          // Create an initial notification confirming receipt of order
          const notifId = "notif_" + Date.now().toString();
          await setDoc(doc(db, "notifications", notifId), {
            id: notifId,
            userId: currentUser?.id || "usr_anonymous",
            title: "✨ تم استلام تصميمك المخصص بنجاح",
            message: `طلبك رقم #${orderId} هو الآن تحت المراجعة الفنية من قِبل "نونا". سنحدد لكِ السعر وموعد الاستلام المناسب قريباً!`,
            orderId: orderId,
            isRead: false,
            createdAt: new Date().toLocaleDateString("ar-EG") + " " + new Date().toLocaleTimeString("ar-EG")
          });
        } catch (dbErr) {
          console.warn("Could not save new order to Firestore, captured locally instead:", dbErr);
        }
      } else {
        ...
        try {
          ...
          const cachedNotifsStr = localStorage.getItem("tuhfa_local_notifications") || "[]";
          const cachedNotifs = JSON.parse(cachedNotifsStr);
          cachedNotifs.unshift(localNotif);
          localStorage.setItem("tuhfa_local_notifications", JSON.stringify(cachedNotifs));
        } catch (e) {
          console.error(e);
        }
            ...
            {!submitted ? (
              ...
              >
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-5">
                  {/* Occasion type */}
                  <div className="flex flex-col gap-1 text-right">
                    <label className="text-xs font-bold text-[#7B3B2A] font-sans">نوع المناسبة الديمقراطية</label>
                    <select
                      value={occasionType}
                  ...
                  <div className="flex flex-col gap-1 text-right">
                    <label className="text-xs font-bold text-[#7B3B2A] font-sans">تنسيق وهيكل الألوان المطلوب</label>
                    <select
                      value={mainColor}
          ...
          {isAdminUnlocked ? (
            <div className="flex items-center gap-2 bg-stone-100/90 p-1.5 rounded-2xl border border-stone-200/50 shadow-inner select-none order-2 md:order-1">
              ...
              <button
                ...
                className={`px-4.5 py-2 rounded-xl text-xs font-sans font-bold transition-all cursor-pointer flex items-center gap-1.5 ${
                  viewMode === "merchant" 
                    ? "bg-[#7B3B2A] text-[#FAF0E8] shadow-sm" 
                    : "text-stone-600 hover:text-[#7B3B2A] hover:bg-stone-50"
                }`}
            ...
            <div className="bg-[#FAF0E8]/50 border-2 border-[#F2C4C4]/40 rounded-[28px] p-6 shadow-xs">
              <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-5 pb-4 border-b border-[#F2C4C4]/20 text-right">
                ...
                <div className="flex items-center gap-2 justify-end font-sans">
                  ...
                  <label className="text-xs font-bold text-[#7B3B2A] font-sans">اختر الفترة الحسابية للتقرير:</label>
                </div>
          ...
          >
            ...
            >
              ...
              <form
                onSubmit={(e) => {
                  e.preventDefault();
                  // Accept multiple easy tokens: "1234", "0000", "nona", "2026", "nonatuhfa"
                  const cleaned = pinInput.trim().toLowerCase();
                  if (["1234", "0000", "2026", "nona", "nonatuhfa"].includes(cleaned)) {
                    setIsAdminUnlocked(true);
